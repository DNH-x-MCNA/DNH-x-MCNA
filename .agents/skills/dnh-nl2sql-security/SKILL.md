...
Checklist này áp dụng bất kể LLM provider nào đang chạy phía sau (`ai_agent/chatbot.py` hiện dùng Gemini/OpenAI làm tạm; kế hoạch chính thức là Codex API — xem `dnh-realtime-etl-pipeline`). Khi thêm nhánh Codex, mọi layer bảo vệ dưới đây vẫn phải giữ nguyên, không viết lại pipeline riêng cho provider mới.
