...
Gọi `/dnh-onsite cập nhật` để cập nhật trạng thái câu hỏi sau khi có câu trả lời từ client (Codex sẽ hỏi lại câu trả lời và ghi vào `references/onsite_qa_log.md`, đồng thời cập nhật `dnh-project-context` nếu câu trả lời ảnh hưởng đến kiến trúc đã chốt).
