...
- **Kiến trúc kỹ thuật chi tiết** (4 phương án lấy dữ liệu realtime, schema `raw_*/stg_*/mart_*`, watermark/upsert, Notification Engine event-driven, Codex API cho chatbot) → xem skill `dnh-realtime-etl-pipeline`, nguồn là `MCNA_DNH_Timeline_v1.2.pdf`. Không lặp lại chi tiết đó ở đây, chỉ giữ bối cảnh tổng quan.
...
- **Bản kỹ thuật chi tiết nhất (ưu tiên dùng)**: `MCNA_DNH_Timeline_v1.2.pdf` (lập 02/07/2026) — lộ trình 3 tháng/12 tuần: T1 W1-2 khảo sát hạ tầng & tạo project Supabase → T1 W3-4 xây ETL lõi (watermark) → T2 W5-6 Notification Engine bản test (Gmail/Telegram) → T2 W7-8 tích hợp Codex API cho chatbot + chuyển kênh production (Graph API/Teams) → T3 W9-10 UAT & go-live → T3 W11-12 giám sát & tối ưu. Chi tiết đầy đủ xem `dnh-realtime-etl-pipeline`.
