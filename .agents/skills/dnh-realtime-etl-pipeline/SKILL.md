...
- Kiến trúc mục tiêu: Supabase Database Webhook (event khi insert/update) → Notification Engine (Python/FastAPI) → xác định loại sự kiện/đối tượng nhận → (tuỳ chọn) gọi Codex API để format nội dung động → gửi qua kênh tương ứng.
...
- Lưu ý kỹ thuật: `Codex-opus-4-8` không nhận `temperature`/`top_p`/`top_k` (400 nếu gửi) — `_call_claude()` cố tình không forward tham số sampling, khác với nhánh Gemini/OpenAI.
- Việc này thuộc phạm vi bắt buộc tuân theo checklist `dnh-nl2sql-security` khi code (multi-layer validation áp dụng bất kể provider nào — không đổi gì ở `_execute_sql`/validator khi thêm Codex).

## Bảo mật & vận hành (mục 7 kế hoạch)

- Secrets (SQL Server conn string, Supabase service key, Codex API key) chỉ qua biến môi trường/secrets manager — không hardcode. Khớp với rủi ro bảo mật đã ghi trong `dnh-project-context`.
...
Xem `references/etl_notification_plan.md` mục "6. Lộ trình" — 3 tháng/12 tuần, 6 giai đoạn (khảo sát hạ tầng → ETL lõi → Notification bản test → tích hợp Codex API + chuyển kênh production → UAT/go-live → giám sát & tối ưu). Timeline này **thay thế/làm chi tiết hơn** mốc "9 tuần" cũ trong `dnh-project-context`; nếu thấy mâu thuẫn giữa hai nguồn, ưu tiên bản 12 tuần này vì mới hơn (lập 02/07/2026).
