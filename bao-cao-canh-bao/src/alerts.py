...
def check_dead_stock_alert():
    ...
    [dbo].[usp_StockLotFinance_Report] — xem docs/bravo_stored_procedures/README.md (do người dùng
    cung cấp toàn văn SP). SP dùng RemainMonths=FLOOR(tồn hiện tại/TB bán 6 tháng gần nhất) >= 6 =
    "Bán chậm" (@_ShortOfStockMonth mặc định), <= 1 = "Thiếu hàng" (@_OutOfStockMonth mặc định).
    Trước đó MCNA tự đặt 12.0 (ước lượng) — nay khớp đúng định nghĩa "bán chậm" của chính DNH.
...
def check_near_expiry_alert():
    ...
    (Ghi chú lịch sử: bản 20/07/2026 từng kết luận "cả Bravo lẫn Supabase đều KHÔNG có cột hạn dùng
    theo lô" — kết luận đó ĐÃ BỊ BÁC BỎ ngày 22/07 nhờ toàn văn SP gốc: Bravo CÓ, chỉ nằm ở bảng
    khác. Vẫn cần DNH/ETL bổ sung đồng bộ BRV_Lot/BRVSX_Lot trước khi bật cảnh báo cận date.)
    """
